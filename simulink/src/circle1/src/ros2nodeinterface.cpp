...
const std::string SLROSNodeName("circle1");
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
		...
		mSchedulerTimer = SLROSNodePtr->create_wall_timer(std::chrono::nanoseconds(500000000),std::bind(&NodeInterface::schedulerThreadCallback,this),mSchedulerGroup);
    }
